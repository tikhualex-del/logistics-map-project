  ...
  try {
    ...
    if (!integration) {
      return NextResponse.json(
        { success: false, message: "No active default integration found" },
        { status: 400 }
      );
    }
    ...
    if (integration.provider !== "retailcrm") {
      return NextResponse.json(
        { success: false, message: `Unsupported provider: ${integration.provider}` },
        { status: 400 }
      );
    }
    ...
    if (!integration.baseUrl) {
      return NextResponse.json(
        { success: false, message: "Integration baseUrl is empty" },
        { status: 400 }
      );
    }
        ...
        batch.map(async (order) => {
          ...
          if (textAddress) {
            try {
              ...
              const entranceFocusedAddress = buildEntranceFocusedGeocodingQueryAddress(order);
              ...
              if (cacheKey) {
                ...
                if (dbCache) {
                  ...
                  if (geocodeResult) {
                    ...
                    await prisma.geocodeCache.upsert({
                      where: { addressKey: cacheKey },
                      update: { lat: null, lon: null, displayName: null, kind: null, precision: null },
                      create: { addressKey: cacheKey, lat: null, lon: null, displayName: null, kind: null, precision: null },
                    });
                  }
                }
              }
